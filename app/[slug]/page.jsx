...
export async function generateMetadata({ params: { slug } }) {
  let { date, excerpt, image, title } = getBlog(slug);
...
export default async function Blog({ params: { slug } }) {
  let { content, date, excerpt, image, tags, title } = getBlog(slug);

  let sourceContent = await serialize(content, {
    mdxOptions: {
      remarkPlugins: [],
      rehypePlugins: [],
    },
  ...
  return (
    <div className="bg-gray-900 max-w-screen-md rounded-xl mx-auto my-8 shadow-2xl">
      ...
      <div className="prose prose-invert prose-img:rounded-xl prose-img:mx-auto prose-img:object-contain prose-img:max-h-96 prose-img:max-w-full max-w-none px-4 sm:px-8 mb-8">
        <BlogMDX sourceContent={sourceContent} />
      </div>
